...
# ---------------------------------------------------------------------------
# EPISODES — CUSTOM (user-generated)
# ---------------------------------------------------------------------------
@app.get("/episodes/custom")
def get_custom_episodes(request: Request, limit: int = 20):
    """Returns custom episodes generated by the logged-in user."""
    user = require_user(request)
    if not user:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    try:
        ...
        cur.execute(
            """
            SELECT id, title, gcs_url, genre, published_at, custom_params
            FROM   episodes
...
def generate_custom_episode(request: Request):
    """
    Generate a custom episode based on user-provided NewsAPI params.
    Body (JSON): { keywords, from_date, to_date, domains, exclude_domains, genre }
    ...
    genre           = body.get("genre", "general").strip()

    # Build NewsAPI URL
    if keywords or domains:
        # Use /everything for keyword/domain searches
        params = {
            "apiKey":   NEWS_API_KEY,
            "pageSize": 10,
            "language": "en",
        ...
        # Use /top-headlines for genre/category
        params = {
            "apiKey":   NEWS_API_KEY,
            "pageSize": 10,
            "country":  "us",
    ...
    # Store custom params + user_id + genre into DB now (episode shell)
    # The TTS worker will fill in gcs_url and transcript later
    try:
        conn = get_db()
        cur  = conn.cursor()
        cur.execute(
            ...
            (
                job_id,
                f"Custom: {keywords or genre} — {datetime.now(timezone.utc).strftime('%Y-%m-%d')}",
                user["id"],
                genre,
                json.dumps({
                    "keywords": keywords,
    ...
    # Push to fetcher queue with custom NewsAPI params
    try:
    ...
    return JSONResponse({
        "status":  "queued",
        "job_id":  job_id,
        "message": "Your podcast is being generated. This takes about 30–60 seconds.",
    })
